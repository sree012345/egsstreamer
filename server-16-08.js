...
// const videoStream = new PassThrough({
//   highWaterMark: 10 * 1024 * 1024, // Set a custom highWaterMark
// });
...
io.on('connection', (socket) => {
  ...
  socket.on('videoChunk', (stream,streamURL) => {
    ...
    if (Buffer.isBuffer(stream)) {
      if (videoStream.writable) {
        if (!videoStream.write(stream)) {
          console.log('Video stream buffer full. Waiting for drain...');
          videoStream.once('drain', () => {
            console.log('Video stream drained. Resuming writing.');
            // Resume writing when the buffer is drained
            videoStream.resume();
          });
        }
      } else {
